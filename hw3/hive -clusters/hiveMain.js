...
// main.js
  ...
  var get_info  = function(data_set, format) {
    ...
    var degree  = Math.PI / 180,
        x_max   = 1000,    x_off   = x_max * 0.9,
        y_max   = 700,    y_off   = y_max * 0.15;

    if (format === 'conv') {  // "conventional"
      var a_off   =   20,
          a_so    =    0,     a_st    = (100 - a_off),
  ...
  var data_sets     = { '#demo_2':   'old 4.json' };
...
var setup_mouse = function(plot_info) {
  ...
  var precis  = d3.select(g.selector + ' .precis')
    .text(default_precis = 'Showing clusters containing classes.  Mouse over any node to see its data interface(s).');
  ...
  // Helper for on_mouseover_{link,node}.
    ...
    if (css_class == 'ib')
      hdr  = '<h4 class="ib">cluster:</h4>';
    else
      hdr  = '<h4 class="im">contains: </h4>';
...
//    if (result) console.log('link_mo', curr_link, orig_link); //T
  ...
  //   the sidebar to show consistent colors and text
    ...
    var html      = '<h3 class="mo">Cluster</h3>'
